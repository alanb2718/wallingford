...
(provide always always* stay wally-clear wally-solve required high medium low lowest
...
; add the primitives
(hash-set! symbolic-struct-creators 'number (lambda () (define-symbolic* x number?) x))
...
(define-syntax wally-define-symbolic
  (syntax-rules (number? boolean?)
    ((wally-define-symbolic vs ... number?) (define-symbolic vs ... number?))
...
(define (wally-solve)
  (define old-required-stay-vals (map evaluate required-stays))
  (define old-soft-stay-vals (map evaluate (map soft-target soft-stays)))
