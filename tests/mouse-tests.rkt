...
(define (mouse-position)
  (test-case
   ...
   (send-thing r1 mouse-event 60 80 56 'up)  ; note this has the same time stamp as the previous event
   (check equal? (get-mp r1) (point 10 20))
   (check equal? (length (send r1 get-mouse-events)) 5)
   (send-thing r1 advance-time 10)
   (check equal? (get-mp r1) (point 10 30))
   (check equal? (length (send r1 get-mouse-events)) 5) ; should still have event for time=0
   ...
   (check equal? (get-mp r1) (point 80 56))
