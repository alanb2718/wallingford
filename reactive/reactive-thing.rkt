...
; integral macro
; helper function to do symbolic integration at compile time - super simple to start with
(define-for-syntax (symbolic-integral expr var)
  (match expr
...
(define-syntax (integral stx)
  (syntax-case stx ()
    [(_ expr)
     ; for now the variable is always (seconds) -- later this should be changeable
     (with-syntax ([intgl (datum->syntax stx (symbolic-integral (syntax->datum #'expr) '(seconds)))]
